...
for k=1:length(density_list)
  ...
  for i=1:length(R_list)
    ...
    for j=1:length(q_list)
      ...
      pqr = struct('xyz',[qLoc-1 0 0; qLoc 0 0],'q',[-1 q]','R',[0 0]');
      ...
      [phiReacYoonDiel, phiBndy,dPhiBndy] = solveConsistentYoonNoSternAsym(surfdata, bemYoonDiel, ...
						  epsIn, epsOut, ...
						  conv_factor, pqr, asymParams);
      L_pcm(i,j,k) = 0.5 * pqr.q'*phiReacPcm;
      L_yoondiel(i,j,k) = 0.5 * pqr.q'*phiReacYoonDiel;
...
errPCM = abs(E_extrap_pcm(i,j)-squeeze(L_pcm(i,j,:)));
loglog(numPoints(i,:),errPCM,'bs', 'linewidth',2);
hold on;
errYL = abs(E_extrap_yl(i,j)-squeeze(L_yoondiel(i,j,:)));
