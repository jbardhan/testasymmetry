...
def memMeshGen2D1S (d0,spacing,df,r_ion,r_mem,h):
    ...
    dataFilesPath = os.path.join(curDir,'I-M-2D1S')
    try:
    ...
    for distance in distRange: #distance between dielectric surface of membrane and ion which changes from 0 (dielectrics touching) to 4 (2*stern_thickness)
        ...
        mergedDielFileName = "dielMerged" 
        mergedSternFileName = "sternMerged"

        np.savetxt("%s.vert" % mergedDielFileName,mergedSternMesh.vertices,fmt='%5.3f',delimiter=' ')
        np.savetxt("%s.face" % mergedDielFileName,elementsMergedStern,fmt='%5.0f',delimiter=' ')
        ...
        # Creating mem.srf and two empty files which run 
        f=open("molmem.srf","w+")
        f.write("f\nf\n./%s\n./%s\n\n\n0\n" % (mergedSternFileName,mergedDielFileName))
        f.close()
        
        
        #mem_Diel = "diel%d" % r
        #mem_Stern = "stern%d" % r
        Path('%s/%s' % (dirPath,mergedDielFileName)).touch()
        Path('%s/%s' % (dirPath,mergedSternFileName)).touch()
