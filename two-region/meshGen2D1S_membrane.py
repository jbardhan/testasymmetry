...
#def memMeshGen2D1S (d0,spacing,df,r_ion,r_mem,h):
def memMeshGen2D1S (distRange,r_ion,r_mem,h,particleCharge):

    #distRange = np.arange(d0,df+0.001,spacing)
    curDir = os.getcwd() #in the two-region folder
    meshFilesPath = os.path.join(curDir,'I-M-2D1S')
    try:
    ...
    memSternMesh = pymesh.load_mesh(sternstl)
    memSternMesh, info = pymesh.remove_duplicated_vertices(memSternMesh)
    memSternMesh, info = pymesh.remove_duplicated_faces(memSternMesh)
    elementsStern = np.copy(memSternMesh.elements)
    ...
    np.savetxt("%s.vert" % mem_Stern,memSternMesh.vertices,fmt='%5.3f',delimiter=' ')
    ...
    memMeshInfo = "%s%d\n%s%d\n%s%d\n%s%d\n%s%d%s" % ('# Elements in dielectric mesh = ',meshDiel.num_elements,
                                           ...
                                           '# Elements in stern layer mesh = ',memSternMesh.num_elements,
                                           '# Vertices in stern layer mesh = ',memSternMesh.num_vertices,
    ...
    for distance in distRange: #distance between dielectric surface of membrane and ion which changes from 0 (dielectrics touching) to 4 (2*stern_thickness)
        ...
        # Cl mesh
        f = open(molDiel,"w+")   
        mol_D_xyzr = "%10.5f%10.5f%10.5f%10.5f\n" % (0,0,h+r_ion+distance,r_ion) # Na-Na or Na-Na(-)
        ...
        # Cl stern mesh
        f = open(molStern,"w+")   
        mol_S_xyzr = "%10.5f%10.5f%10.5f%10.5f\n" % (0,0,h+r_ion+distance,r_ion+2) # Na-Na or Na-Na(-)
        ...
        #listDir = os.listdir(meshFilesPath)
        #threeFirstLines = []      
        dielFaceVert_v = "mol_diel_%i.vert" % count
        dielFaceVert_f = "mol_diel_%i.face" % count
        sternFaceVert_v = "mol_stern_%i.vert" % count
        sternFaceVert_f = "mol_stern_%i.face" % count
        '''for item in [dielFaceVert_v,dielFaceVert_f,sternFaceVert_v,sternFaceVert_f]:
...
'''
        ...
        with open(sternFaceVert_v) as f_vert:
        ...
        with open(sternFaceVert_f) as f_face:
        ...
        molSternMesh = pymesh.form_mesh(vert_mol, face_mol)


        #mesh_mem = pymesh.load_mesh("stern15.stl")
        mergedSternMesh = pymesh.boolean(molSternMesh, memSternMesh,operation="union", engine = "cork")
        mergedSternMesh, info = pymesh.collapse_short_edges(mergedSternMesh, abs_threshold=0.2 , preserve_feature=False) ##### New
        mergedSternMesh, info =pymesh.remove_obtuse_triangles(mergedSternMesh, max_angle=130, max_iterations=5)
        mergedSternMesh, info = pymesh.collapse_short_edges(mergedSternMesh, abs_threshold=0.25 , preserve_feature=False) ##### New
        
        #mergedSternMesh = pymesh.subdivide(mergedSternMesh, order=2, method="loop")
        mergedSternFileName = "sternMerged_%i" % count
        mergedSternFileNameStl = "sternMerged_%i.stl" % count
        pymesh.save_mesh(mergedSternFileNameStl,mergedSternMesh)
        #pymesh.save_mesh("ali.stl", output_mesh);
        elementsMergedStern = np.copy(mergedSternMesh.elements)
        #Because element numbers in PyMesh start from zero!
        for i in range(elementsMergedStern.shape[0]):
            for j in range(elementsMergedStern.shape[1]):
                elementsMergedStern[i,j]+=1
            
        mergedSternFileName = "sternMerged_%i" % count

        np.savetxt("%s.vert" % mergedSternFileName,mergedSternMesh.vertices,fmt='%5.3f',delimiter=' ')
        np.savetxt("%s.face" % mergedSternFileName,elementsMergedStern,fmt='%5.0f',delimiter=' ')
        
        # Creating mem.srf and two empty files which run 
        mergedSurfFule = "molMemMerged_%i.srf" % count
        f=open(mergedSurfFule,"w+")
        f.write("f\nf\n./%s\n./%s\n\n\n0\n" % (mergedSternFileName,mergedSternFileName))
        f.close()
        
        
        #mem_Diel = "diel%d" % r
        #mem_Stern = "stern%d" % r
        Path('%s/%s' % (meshFilesPath,mergedSternFileName)).touch()
        count += 1
        os.chdir(meshFilesPath)
