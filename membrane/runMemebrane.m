...
% Radius of the last cylinder to run SLIC (starting from R=1 Angstrom) 
lastCylRadiusToCalc = 4;
...
for j=1:length(cylinderRadius)
    dir=sprintf('%s/%d',curdir,cylinderRadius(j));
    chdir(dir);
    for i=1:length(lambdaFEP)
    ...
    membraneFileName = sprintf('RunMembrane_rad_%d',cylinderRadius(j));
