...
for i=1:length(unique_atom_types_solvent)
    ...
    square_well_exp_solvent = coeff_solvent*exp(beta*coeff_solvent)*(lambda^3-1);
    ...
    for j=1:length(unique_atom_types_solute)
        ...
        square_well_exp_solute = radii^3*coeff_solute*exp(beta*coeff_solute)*(lambda^3-1);
    ...
    dispE = dispE + m_tau_solvent*sqrt(square_well_exp_solvent)*dispSoluteSolvent;
