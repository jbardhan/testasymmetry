...
for kk=1:tempdiv
    ...
    tempKelvin=TEMP(kk)+KelvinOffset;
    ...
    [dG_list,error_code] = ...
	determinePaluchSolvationFreeEnergy('reference-data/mobley_paluch_water.csv',testset,tempKelvin);
    %return;
    if error_code > 0
    ...
    % 
    x0 = [0.476  -121.417  -1.392  -0.590  0.100  0.005  0.079]; %from SLIC w/o ion for water 298
    if ionflag==0
        if paramboundflag==1
    ...
    es0vec(kk,:)=es0;
    np0vec(kk,:)=np0;
    tempvec(kk,:)=tempKelvin;
    epsvec(kk,:)=epsOut;
end
if ionflag==0
    if paramboundflag==1
        save('OptWater_wo_ion','xvec','refvec','calcvec','esvec','npvec','x0vec','calc0vec','es0vec','np0vec','tempvec','epsvec');
    else
        save('OptWaterPaluch_wo_ion_wo_bound','xvec','refvec','calcvec','esvec','npvec','x0vec','calc0vec','es0vec','np0vec','tempvec','epsvec');
    end
elseif ionflag==1
    if paramboundflag==1
        save('OptWaterPaluch_w_ion','xvec','refvec','calcvec','esvec','npvec','x0vec','calc0vec','es0vec','np0vec','tempvec','epsvec');
    else
        save('OptWaterPaluch_w_ion_wo_bound','xvec','refvec','calcvec','esvec','npvec','x0vec','calc0vec','es0vec','np0vec','tempvec','epsvec');
    end
end
