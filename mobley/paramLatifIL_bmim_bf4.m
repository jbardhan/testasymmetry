...
epsIn  =  1;
Tbase = 300; 
epsOut = 11.7; % from Wakai,Ott,Weingartner 2005 (JPCB Lett, v 109:17028)
...
fid = fopen('reference-data/mobley_latif_bmim_bf4.csv','r'); 
Data = textscan(fid,'%s %f %f','delimiter',',');
fclose(fid);
mol_list = Data{1};
dG_list = Data{3}*convertKJtoKcal;
%%%%%%%%%%%%%%%%%  
% for Temp = 328, set Temp = Data{4}(1) and
%CHANGE THE ABOVE LINE to dG= Data{5} * kB * Data{4}(1) 
%%%%%%%%%%%%%%%%%
...
testset = {'methane','propane','n_butane','2_methylpropane','toluene','methanol','ethanol',...
	   'p_cresol','methanethiol','methyl_ethyl_sulfide',...
	   'ethanamide','3_methyl_1h_indole'};
...
lb = [-2 -200 -100 -20  -0.1  -0.2  -2.5];
ub = [+2 +200 +100 +20  +0.1  +0.2  +2.5];
...
save('OptLatifBMIM_BF4','x','ref','calc','es','np','x0','calc0','es0','np0');
